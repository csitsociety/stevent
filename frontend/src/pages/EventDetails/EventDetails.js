...
import { Heading, Paragraph as P, Pill, Spinner, Toggle } from 'components'
import {
  getEventDetails,
  retrieveClubs,
  retrieveAttendees,
...
const EventDetails = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      if (id) {
        const details = await getEventDetails({ eventID: id })
        setEvent(details.event)

        setClubs((await retrieveClubs()).clubs)

        const eventAttendees = (await retrieveAttendees({ eventID: id }))
          .attendingUsers
        if (eventAttendees.find((u) => u.id === profile.id)) {
          setGoing(1)
        } else {
          setGoing(0)
        }
